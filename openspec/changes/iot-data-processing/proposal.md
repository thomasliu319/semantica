...
状态：`spec-frozen`（B1–D5、E1/E2）。已写入 [decisions.md](./decisions.md)，规格见 [specs/iot-data-processing/spec.md](./specs/iot-data-processing/spec.md)。实现任务见 [tasks.md](./tasks.md)。
...
结构化拉取已经完成 **199 / 200** 台（T-V856S 100、T-600 99），窗口 2026-03-01～08-31。  
`172609583` 无台账身份，永久排除。原始 JSON 不能直接进特征或模型：粒度混杂、空字段多、部分接口会串机或串客户，3 月几乎全停。
...
1. **只收磁盘完备设备（199 台）**；`172609583` 不进清洗集。
...
来源：199 台完备目录 `datasets/json/iot_timeseries/<OutFactoryCode>/`，窗口 Mar–Aug 2026。语义层见 [metrics.md](./metrics.md)。

| 事实 | 数字 |
|---|---|
| 完备 / Excel | 199 / 200 |
| T-V856S / T-600 | 100 / 99 |
| 排除 | `172609583`（identity 空） |
| 客户数 | 108 |
| 4–8 月运行小时 | 76,800；单台中位有运行日 21 / 153 |
| 3 月几乎全停 | 业务日从 4 月起（B5） |
